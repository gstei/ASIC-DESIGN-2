...
\label{sec:losses}
The total conversion losses can classified into two broad categories of switching frequency dependent switching losses and load dependent conduction losses as shown in \autoref{eq:PLoss}. Our measured conversion losses can be seen in \autoref{fig:losses} and shows a roughly \qty{100}{\milli\watt} load independent loss, which explains the poor efficiency at sub \qty{100}{\milli\ampere} output currents. As the output currents increase, the proportion of power loss due to switching decreases, leading to the improved efficiency in the 100 to \qty{200}{\milli\ampere} output current range. With large output currents >\qty{200}{\milli\ampere} the conduction losses take over leading again to a decrease in efficiency.

\begin{equation}
    P_{TOT} \approx P_{SWITCH} + P_{COND}
\end{equation}
\label{eq:PLoss}
For an iso-frequency measurement as conducted in \autoref{fig:losses} the switching losses appear as a constant offset as $P_{SWITCH}\;\propto\;1$ and the conduction losses increase with the load current as $P_{COND}\;\propto\;I_{OUT}^2$. These contributions can be clearly seen \autoref{fig:losses} as a constant loss together with a quadratically increasing loss based on the load applied.
